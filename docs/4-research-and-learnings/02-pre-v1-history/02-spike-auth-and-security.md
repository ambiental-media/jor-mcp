...
2.  **Rate Limiting:** To prevent abuse, rate limiting is mandatory. Because the Cloud Run deployment will be stateless and horizontally scalable, an in-memory rate limiter is insufficient. We will utilize **GCP Memorystore (Redis)** to implement a distributed sliding-window rate limit.
3.  **Authorization (Tiers):** By utilizing Firebase Auth, we can inject custom claims into the JWT (e.g., `tier: basic` or `tier: pro`). The Redis rate limiter will read these claims to apply differentiated traffic quotas.