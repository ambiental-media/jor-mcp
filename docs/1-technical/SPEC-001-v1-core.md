...
- **Robust Security:** Implement OAuth 2.0 authentication using Firebase Auth / Google Identity Platform and distributed rate limiting via GCP Memorystore (Redis).
...
The system relies on a stateless FastMCP ASGI application hosted on Google Cloud Run, backed by GCP Memorystore (Redis) for distributed rate-limiting.
...
4. The `RateLimitMiddleware` checks the Redis cache against the user's quota tier.
...
- [ ] Initialize project dependencies via `uv` (`fastmcp`, `uvicorn`, `redis`, `firebase-admin`, `httpx`, `opentelemetry`).
- [ ] Implement `AuthMiddleware` leveraging `firebase-admin` for JWT validation.
- [ ] Implement `RateLimitMiddleware` utilizing `redis.asyncio` and GCP Memorystore, supporting tiered quotas based on Firebase custom claims.
...
- [ ] Instrument the application with OpenTelemetry (ASGI, HTTPX, Redis).
- [ ] Write exhaustive tests (`pytest-asyncio`) mocking Redis, Firebase, and external APIs to achieve >90% coverage.
